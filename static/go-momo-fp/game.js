(() => {
  ...
  for (let i = 0; i < 34; i++) {
    const a = i * 1.7;
    posts.push({ x: Math.cos(a) * (5 + (i % 5) * 2.4) + (i % 3) * 0.5, y: Math.sin(a) * (5 + (i % 7) * 1.9), h: 1.1 + (i % 4) * 0.15, done: false, hit: 0 });
  }
  function wrapPi(a) {
  ...
  function stepMomo(dt) {
    ...
    let tx, ty;
    if (momo.target) { tx = momo.target.x; ty = momo.target.y; }
    else {
    ...
    if (momo.target && atTarget && momo.sniff > 0.8 && !momo.target.done) {
      momo.target.done = true; momo.target.hit = 1; mail++; momo.think = 0.2;
    }
  ...
  function step(dt) {
    ...
    const turn = (pad.right ? 1 : 0) - (pad.left ? 1 : 0);
    you.heading += turn * dt * 1.7;
  ...
  function drawDpad() {
    ...
    function tri(x, y, rot) {
      dctx.save(); dctx.translate(x, y); dctx.rotate(rot);
      dctx.beginPath(); dctx.moveTo(0, -9); dctx.lineTo(8, 6); dctx.lineTo(-8, 6); dctx.closePath(); dctx.fill(); dctx.restore();
    }
    dctx.fillStyle = pad.up ? lit : dim; tri(cx, cy - arm * 1.5, 0);
    dctx.fillStyle = pad.down ? lit : dim; tri(cx, cy + arm * 1.5, Math.PI);
    dctx.fillStyle = pad.left ? lit : dim; tri(cx - arm * 1.5, cy, -Math.PI / 2);
    dctx.fillStyle = pad.right ? lit : dim; tri(cx + arm * 1.5, cy, Math.PI / 2);
  }
  function drawCrank() {
    ...
    cctx.beginPath(); cctx.roundRect(-6, -7, r * 0.95, 14, 7); cctx.fill();
  ...
  function drawWorld() {
    ditherSky();
    const ordered = posts.map(p => ({ p, z: worldToCam(p.x, p.y).z })).filter(o => o.z > 0.35).sort((a, b) => b.z - a.z);
    ...
    if (mp) {
      ...
      const cy2 = mp.y - 6;
      lctx.beginPath(); lctx.moveTo(handX, handY);
      if (leash.taut > 0.5) lctx.lineTo(cx2, cy2);
      else {
      ...
      lctx.beginPath(); lctx.moveTo(handX, handY); lctx.lineTo(side, LCD_H - 46); lctx.stroke();
    }
    lctx.fillStyle = INK;
  ...
  function frame(now) {
    ...
    tPrev = now;
    step(dt); drawWorld(); drawCrank(); drawDpad();
